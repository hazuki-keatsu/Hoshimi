...
- **高性能资源优化 (High-Performance Assets)**:
    - **内存池 (Memory Pool)**: 为大尺寸纹理（背景、立绘）预分配内存桶，切换场景时直接复用内存块，消除频繁申请/释放产生的内存碎片与时延。
    - **剧本预编排 (Resource Pre-sorting)**: 打包工具通过静态分析剧本流向，将资源在二进制文件(`.pak`)中按剧情顺序线性排布，配合 `mmap` 实现读取时的零指针跳转与近乎零拷贝的极速加载。
